...
class PixelRequest
  ...
  def visitor_browser
    Visitor.browsers[parsed_user_agent.family.downcase] || "other"
  end
  ...
  def visitor_device_type
    ...
    when "android", "ios" then "phone"
    when "windows", "linux", /mac os/  then "desktop"
    end
  ...
  def parsed_user_agent
    @parsed_user_agent ||= USER_AGENT_PARSER.parse(user_agent).tap do |p|
      TinyPixel.research_log.info "User-agent: #{p.to_h.inspect}"
    end
  end
end
